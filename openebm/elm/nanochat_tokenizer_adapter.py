...
class RustBPETokenizer:
    ...
    def __init__(self, enc):
        self._enc = enc
        # enc が nanochat RustBPETokenizer（encode_special メソッドを持つ）の場合は
        # _special_tokens dict は不要 — メソッドに直接委譲する
        self._delegate = hasattr(enc, 'encode_special') and hasattr(enc, 'get_bos_token_id')
        if not self._delegate:
            # tiktoken Encoding を直接ラップする場合
            if hasattr(enc, '_special_tokens') and isinstance(enc._special_tokens, dict):
    ...
    def get_vocab_size(self):
        if self._delegate and hasattr(self._enc, 'get_vocab_size'):
            return self._enc.get_vocab_size()
        return self._enc.n_vocab

    def get_bos_token_id(self):
        if self._delegate:
            return self._enc.get_bos_token_id()
    ...
    def encode_special(self, special_token_name):
        if self._delegate:
            return self._enc.encode_special(special_token_name)
